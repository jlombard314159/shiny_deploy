function(input, output){
    ...
    plot_title_time = reactive({
        paste0("Rate of ", input$selected_outcome_time, " 2021 - 2023") 
    })
    
    output$output_plotlyTable = renderPlotly({
        
        johns_new_plot(data_time(), title = plot_title_time())
    ...
    ###############  
    data_map = reactive({
        datasets[[input$year_map]] %>% 
            filter(Health_Indicator_Desc == input$selected_outcome_map) %>%
            filter(County != "CALIFORNIA")
    }) 
    
    output$map = renderLeaflet({
        
        makeleafletMap(data_map(), cal_counties_geo, var = "Rate.Percentage") %>%
            addControl(input$selected_outcome_map, position = "bottomleft")
